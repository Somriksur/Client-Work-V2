...
export function DesktopWhoThisIsFor() {
  ...
  const [reason1] = useState('Meta Partner Certified');
  const [reason2] = useState('10+ Years Of Proven Experience');
  const [reason3] = useState('Revenue-Focused Marketing');
  const [reason4] = useState('Custom Strategies, No Templates');
  const [reason5] = useState('Expert-Led Campaign Execution');
  const [reason6] = useState('Results In Competitive Markets');
